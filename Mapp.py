...
def page_materials(materials_section: str | None = None) -> None:
    ...
    if materials_section == "Add Material":
        ...
        if cats.empty or units.empty:
            ...
            add_edit_mode = st.radio(
                "Material Action",
                ["Add Material", "Edit Material"],
                horizontal=True,
                key="materials_add_edit_mode",
            )

            if add_edit_mode == "Add Material":
                ...
                st.caption("Edit the material record and the vendor information for the selected material.")
                search_value = st.text_input(
                    "Search by item number or material name",
                    key="add_material_edit_search",
                )
                if not search_value.strip():
                    st.info("Enter an item number or material name to locate a material.")
                else:
                    results_df = search_material_selector(search_value.strip(), limit=100)
                    if results_df.empty:
                        st.warning("No materials matched that search.")
                    else:
                        result_options = {
                            f"{row['display_label']} | Material ID {int(row['material_id'])}": int(row['material_id'])
                            for _, row in results_df.iterrows()
                        }
                        selected_label = st.selectbox(
                            "Select material",
                            list(result_options.keys()),
                            key="add_material_edit_select",
                        )
                        selected_material_id = result_options[selected_label]
                        material_row = query_one("SELECT * FROM materials WHERE material_id = %s", (selected_material_id,))
                        if material_row is None:
                            st.error("Could not load that material.")
                        else:
                            cats_all = lookups["active_categories"].copy()
                            subs_all = lookups["subcategories"].copy()
                            units_all = lookups["units"].copy()
                            ...
                            current_category_name = next(
                                (name for name, cid in category_options_edit.items() if cid == material_row.get("category_id")),
                                "No Category",
                            )
                            default_category_index = category_labels.index(current_category_name) if current_category_name in category_labels else 0
                            selected_edit_category_name = st.selectbox(
                                ...
                                category_labels,
                                index=default_category_index,
                                key="add_material_edit_category",
                            )
                            selected_category_id = category_options_edit[selected_edit_category_name]
                            ...
                            for _, row in filtered_subs.iterrows():
                                sub_options_edit[str(row["subcategory_name"])] = int(row["subcategory_id"])
                            sub_labels = list(sub_options_edit.keys())
                            current_sub_name = next(
                                (name for name, sid in sub_options_edit.items() if sid == material_row.get("subcategory_id")),
                                "No Sub-category",
                            )
                            default_sub_index = sub_labels.index(current_sub_name) if current_sub_name in sub_labels else 0
                            ...
                            for _, row in units_all.iterrows():
                                unit_options_edit[str(row["unit_name"])] = int(row["unit_id"])
                            unit_labels = list(unit_options_edit.keys())
                            current_unit_name = next(
                                (name for name, uid in unit_options_edit.items() if uid == material_row.get("default_unit_id")),
                                "No Default Unit",
                            )
                            default_unit_edit_index = unit_labels.index(current_unit_name) if current_unit_name in unit_labels else 0

                            with st.form("add_material_page_edit_material_form"):
                                e1, e2 = st.columns(2)
                                edit_material_name = e1.text_input("Material name", value=material_row.get("material_name") or "")
                                edit_internal_code = e2.text_input("Internal material code", value=material_row.get("internal_material_code") or "")
                                edit_full_description = st.text_area("Full description", value=material_row.get("full_description") or "")
                                e3, e4 = st.columns(2)
                                edit_sub_name = e3.selectbox("Sub-category", sub_labels, index=default_sub_index, key="add_material_edit_sub")
                                edit_unit_name = e4.selectbox("Default unit", unit_labels, index=default_unit_edit_index, key="add_material_edit_unit")
                                e5, e6, e7 = st.columns(3)
                                edit_manufacturer = e5.text_input("Manufacturer", value=material_row.get("manufacturer") or "")
                                edit_model_number = e6.text_input("Model number", value=material_row.get("model_number") or "")
                                edit_dimension_display = e7.text_input("Dimensions display", value=material_row.get("dimension_display") or "")
                                edit_notes = st.text_area("Notes", value=material_row.get("notes") or "", key="add_material_edit_notes")
                                edit_active = st.checkbox("Active", value=bool(material_row.get("active", 1)), key="add_material_edit_active")
                                save_material_changes = st.form_submit_button("Save Material Changes")
                                if save_material_changes:
                                    if not edit_material_name.strip():
                                        st.error("Material name is required.")
                                    else:
                            ...
                            st.markdown("**Vendor records for selected material**")
                            ...
                            if vendor_edit_df.empty:
                                ...
                                selected_vendor_label = st.selectbox(
                                    ...
                                    key="add_material_edit_vendor_record_select",
                                )
                                selected_vendor_id = vendor_options_edit[selected_vendor_label]
                                vendor_row = query_one(
                                    "SELECT * FROM material_vendor_current WHERE material_vendor_id = %s",
                                    (selected_vendor_id,),
                                )
                                with st.form("add_material_page_edit_vendor_record_form"):
                                    ...
                                    v6, v7 = st.columns(2)
                                    edit_latest_retail_price = v6.number_input("Latest retail price", min_value=0.0, step=0.01, value=float(vendor_row.get("latest_retail_price") or 0.0), key="add_material_edit_latest_retail_price")
                                    retail_date_default = _safe_date_value(vendor_row.get("latest_retail_price_date")) or datetime.now().date()
                                    edit_latest_retail_price_date = v7.date_input("Retail price date", value=retail_date_default, format="MM/DD/YYYY", key="add_material_edit_latest_retail_price_date")
                                    v8, v9 = st.columns(2)
                                    edit_latest_quoted_price = v8.number_input("Latest quoted price", min_value=0.0, step=0.01, value=float(vendor_row.get("latest_quoted_price") or 0.0), key="add_material_edit_latest_quoted_price")
                                    quoted_date_default = _safe_date_value(vendor_row.get("latest_quoted_price_date")) or datetime.now().date()
                                    edit_latest_quoted_price_date = v9.date_input("Quoted price date", value=quoted_date_default, format="MM/DD/YYYY", key="add_material_edit_latest_quoted_price_date")
                                    edit_vendor_active = st.checkbox("Vendor record active", value=bool(vendor_row.get("active", 1)), key="add_material_edit_vendor_active")
                            ...
                            with st.form("add_material_page_add_vendor_record_form"):
                                st.markdown("**Add vendor record for selected material**")
                                n1, n2, n3 = st.columns(3)
                                new_vendor_name = n1.text_input("Vendor name", key="add_material_new_vendor_name")
                                new_vendor_item_number = n2.text_input("Vendor item number", key="add_material_new_vendor_item_number")
                                new_vendor_store_number = n3.text_input("Store number", key="add_material_new_vendor_store_number")
                                n4, n5 = st.columns(2)
                                new_store_aisle = n4.text_input("Store aisle", key="add_material_new_store_aisle")
                                new_vendor_notes = n5.text_input("Vendor notes", key="add_material_new_vendor_notes")
                                n6, n7 = st.columns(2)
                                new_latest_retail_price = n6.number_input("Latest retail price", min_value=0.0, step=0.01, key="add_material_new_latest_retail_price")
                                new_latest_retail_price_date = n7.date_input("Retail price date", value=datetime.now().date(), format="MM/DD/YYYY", key="add_material_new_latest_retail_price_date")
                                n8, n9 = st.columns(2)
                                new_latest_quoted_price = n8.number_input("Latest quoted price", min_value=0.0, step=0.01, key="add_material_new_latest_quoted_price")
                                new_latest_quoted_price_date = n9.date_input("Quoted price date", value=datetime.now().date(), format="MM/DD/YYYY", key="add_material_new_latest_quoted_price_date")
                                add_vendor_record = st.form_submit_button("Add Vendor Record")
                                if add_vendor_record:
                                    if not new_vendor_name.strip():
                                        ...
                                        execute(
                                            ...
                                            (
                                                selected_material_id,
                                                new_vendor_name.strip(),
                                                normalize_vendor_item_number(new_vendor_item_number) or None,
                                                new_vendor_store_number.strip() or None,
                                                new_store_aisle.strip() or None,
                                                new_latest_retail_price,
                                                new_latest_retail_price_date,
                                                new_latest_quoted_price,
                                                new_latest_quoted_price_date,
                                                new_vendor_notes.strip() or None,
                                                1,
                                                now_ts(),
                                            ),
                                        )
                                        st.success("Vendor record added.")
                                        st.rerun()
