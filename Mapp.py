...
def page_projects(project_section: str | None = None) -> None:
    ...
    if project_section == "Add Materials to Work Item":
        ...
        if work_items.empty or units.empty:
            st.info("You need at least one project, sub-project, and unit.")
        else:
            ...
            st.caption("Search by vendor item number or material name, then choose a result to populate the line.")
            search_term = st.text_input("Search material by item number or name", key="material_search_selector_term").strip()

            selected_material_id = None
            selected_material_name = None
            selected_vendor_id = None
            selected_vendor_label = "None"
            default_price = 0.0
            default_unit_name = unit_choices[0] if unit_choices else ""
            ...
            if search_term:
                ...
                search_results = query_df(
                    ...
                    SELECT
                        ...
                        COALESCE(mvc.material_vendor_id, 0) AS material_vendor_id,
                        COALESCE(mvc.vendor_name, '') AS vendor_name,
                        COALESCE(mvc.vendor_item_number, '') AS vendor_item_number,
                        COALESCE(mvc.latest_retail_price, 0) AS latest_retail_price,
                        COALESCE(mvc.latest_quoted_price, 0) AS latest_quoted_price
                    FROM materials m
                    LEFT JOIN company_categories c ON m.category_id = c.category_id
                    LEFT JOIN company_subcategories s ON m.subcategory_id = s.subcategory_id
                    LEFT JOIN material_vendor_current mvc
                        ON m.material_id = mvc.material_id
                       AND mvc.active = 1
                    WHERE m.active = 1
                      AND (
                    ...
                    ORDER BY
                        CASE
                            WHEN COALESCE(mvc.vendor_item_number, '') ILIKE %s THEN 0
                            WHEN m.material_name ILIKE %s THEN 1
                            ELSE 2
                        END,
                        m.material_name,
                        mvc.vendor_name,
                    ...
                    LIMIT 100
                    """,
                    (like, like, like, like, search_term + "%", like),
                )
                ...
                if search_results.empty:
                    ...
                    for _, row in search_results.iterrows():
                        vendor_name = str(row.get("vendor_name") or "No Vendor")
                        vendor_item_number = normalize_vendor_item_number(row.get("vendor_item_number"))
                        price_value = row.get("latest_quoted_price")
                        if pd.isna(price_value) or float(price_value or 0) <= 0:
                            price_value = row.get("latest_retail_price")
                        label = (
                            f"{row['material_name']} | {vendor_name} | "
                            f"#{vendor_item_number or 'No Item #'} | "
                            f"{money(price_value)} | "
                            f"{row['category_name'] or 'No Category'}"
                        )
                    ...
                    selected_result_label = st.selectbox(
                        "Matching materials",
                        option_labels,
                        key="material_search_selector_choice",
                    )
                    selected_row = option_map[selected_result_label]
                    selected_material_id = int(selected_row["material_id"])
                    selected_material_name = str(selected_row["material_name"])
                    vendor_id_raw = selected_row.get("material_vendor_id", 0)
                    selected_vendor_id = int(vendor_id_raw) if pd.notna(vendor_id_raw) and int(vendor_id_raw) > 0 else None
                    selected_vendor_label = (
                        f"{selected_row.get('vendor_name') or 'No Vendor'}"
                        + (f" | {normalize_vendor_item_number(selected_row.get('vendor_item_number'))}" if str(selected_row.get("vendor_item_number") or "").strip() else "")
                    )
                    price_value = selected_row.get("latest_quoted_price")
                    if pd.isna(price_value) or float(price_value or 0) <= 0:
                        price_value = selected_row.get("latest_retail_price")
                    default_price = float(price_value or 0.0)
                    default_unit_id = selected_row.get("default_unit_id")
                    if pd.notna(default_unit_id) and int(default_unit_id) in unit_name_by_id:
                        default_unit_name = unit_name_by_id[int(default_unit_id)]

                    st.caption("Selected material")
                    st.write(f"**Material:** {selected_material_name}")
                    st.write(f"**Vendor record:** {selected_vendor_label}")
                    st.write(f"**Default unit:** {default_unit_name}")
                    st.write(f"**Default price:** {money(default_price)}")
            else:
                st.info("Type an item number or material name to load matching materials.")

            with st.form("add_material_to_work_item_form"):
                work_item_label = st.selectbox("Sub-Project", list(wi_map.keys()))
                st.text_input("Selected Material", value=selected_material_name or "", disabled=True)
                st.text_input("Selected Vendor Record", value=selected_vendor_label, disabled=True)
                c1, c2, c3 = st.columns(3)
                ...
                if submitted:
                    if not selected_material_id:
                        st.error("Search and select a material first.")
                    else:
...
def main() -> None:
    ...
    page_labels = [
        ...
        "Projects - Add Work Item",
        "Projects - Add Materials to Work Item",
    ...
    page_dispatch = {
        ...
        "Projects - Add Work Item": (page_projects, "Add Work Item"),
        "Projects - Add Materials to Work Item": (page_projects, "Add Materials to Work Item"),
