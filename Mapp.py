...
def render_material_search_results(df: pd.DataFrame) -> None:
    ...
    display_df = df[["material_id", "material_name", "vendor", "item_number", "full_description"]].copy()
    ...
    st.markdown("**Search Result Photos**")
    for _, row in df.iterrows():
        material_id = int(row["material_id"])
        photo_bytes = get_primary_material_photo(material_id)
        c_photo, c_details = st.columns([1, 5])
        with c_photo:
            if photo_bytes:
        ...
        with c_details:
            st.markdown(
                f"**ID {material_id} | {row.get('material_name') or ''}**  \n"
                f"Vendor: {row.get('vendor') or ''}  \n"
                f"Item #: {row.get('item_number') or ''}  \n"
                f"{row.get('full_description') or ''}"
            )
        st.divider()
            ...
            if not vendor_edit_term:
                ...
                if vendor_edit_all_df.empty:
                    ...
                    with st.form(f"add_vendor_info_edit_form_{selected_vendor_id}"):
                        ...
                        v4, v5 = st.columns(2)
                        edit_store_aisle = v4.text_input("Store aisle", value=vendor_row.get("store_aisle") or "", key=f"add_vendor_info_edit_store_aisle_{selected_vendor_id}")
                        edit_vendor_notes = v5.text_input("Vendor notes", value=vendor_row.get("vendor_notes") or "", key=f"add_vendor_info_edit_notes_{selected_vendor_id}")
                        v6, v7 = st.columns(2)
                        edit_latest_retail_price = v6.number_input(
                            "Latest retail price",
                            min_value=0.0,
                            step=0.01,
                        ...
                        edit_latest_retail_price_date = v7.date_input(
                            "Retail price date",
                        ...
                        v8, v9 = st.columns(2)
                        edit_latest_quoted_price = v8.number_input(
                            "Latest quoted price",
                            min_value=0.0,
                            step=0.01,
                        ...
                        edit_latest_quoted_price_date = v9.date_input(
                            "Quoted price date",
    ...
    if materials_section == "Search / Review":
        ...
        if run_search:
            ...
            if search.strip():
                ...
                """
                params.extend([like, like, like, like, like, like, like, digits_like, like])
