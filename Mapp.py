...
def render_material_photo_manager(material_id: int, key_prefix: str) -> None:
    with st.expander("Material Photos", expanded=False):
        st.caption("Add product photos by uploading image files, dragging screenshots into the box, or pasting a copied screenshot into the upload area when your browser supports clipboard paste.")
        uploaded_photos = st.file_uploader(
            "Add photo(s) / screenshot(s) for this material",
            type=["png", "jpg", "jpeg", "webp"],
            accept_multiple_files=True,
            key=f"{key_prefix}_photo_upload",
            help="Use Windows Snipping Tool or Print Screen, then paste into this upload area if supported. You can also drag/drop or browse for the screenshot image file.",
        )
        photo_caption = st.text_input(
            "Photo caption / notes",
            key=f"{key_prefix}_photo_caption",
        )
        if st.button("Save Photo(s)", key=f"{key_prefix}_save_photos"):
    ...
    if materials_section == "Add Material":
        ...
        if cats.empty or units.empty:
            ...
            with st.form("add_material_form"):
                ...
                st.caption("Material photos can be uploaded, dragged into the box, or pasted from a copied screenshot when your browser supports clipboard paste.")
                material_photos = st.file_uploader(
                    "Material photo(s) / screenshot(s)",
                    type=["png", "jpg", "jpeg", "webp"],
                    accept_multiple_files=True,
