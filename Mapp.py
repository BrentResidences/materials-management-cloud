...
def delete_material_group_from_subproject(work_item_material_id: int) -> int:
    line_row = query_one(
        "SELECT * FROM work_item_materials WHERE work_item_material_id = %s",
        (work_item_material_id,),
    )
    if not line_row:
        return 0
    ...
    row = query_one(
        ...
        WHERE work_item_id = %s
          ...
          AND material_vendor_id IS NOT DISTINCT FROM %s
          AND unit_id IS NOT DISTINCT FROM %s
          AND unit_price IS NOT DISTINCT FROM %s
        """,
        (
            line_row["work_item_id"],
            line_row.get("material_id"),
            line_row.get("material_vendor_id"),
            line_row.get("unit_id"),
            line_row.get("unit_price"),
        ),
    )
    count = int(row["c"] if row else 0)
    if count <= 0:
...
def merge_matching_material_lines(work_item_material_id: int) -> None:
    line_row = query_one(
        "SELECT * FROM work_item_materials WHERE work_item_material_id = %s",
        (work_item_material_id,),
    )
    ...
    if matches_df.empty or len(matches_df) <= 1:
        return

    keep_id = int(matches_df.iloc[0]["work_item_material_id"])
    total_qty = float(pd.to_numeric(matches_df["quantity"], errors="coerce").fillna(0).sum())
    unit_price = float(matches_df.iloc[0].get("unit_price") or 0)
    merged_total = total_qty * unit_price
    merged_notes = " | ".join([str(n).strip() for n in matches_df["notes"].fillna("") if str(n).strip()])
    ...
    execute(
        ...
        WHERE work_item_material_id = %s
        """,
        (total_qty, merged_total, merged_notes or None, now_ts(), keep_id),
    )

    duplicate_ids = [int(v) for v in matches_df["work_item_material_id"].tolist() if int(v) != keep_id]
    if duplicate_ids:
        placeholders = ", ".join(["%s"] * len(duplicate_ids))
        execute(
            f"DELETE FROM work_item_materials WHERE work_item_material_id IN ({placeholders})",
            tuple(duplicate_ids),
        )
        ...
        if projects_df.empty or units.empty:
            ...
            if subprojects_df.empty:
                ...
                if current_lines_df.empty:
                    st.caption(f"0 item(s) | Total {money(current_total)}")
                    st.info("No items have been entered into this Sub-Project yet.")
                else:
                    current_items_display_df = current_lines_df.copy()
                    current_items_display_df["item_number"] = current_items_display_df["item_number"].fillna("")
                    current_items_display_df["material"] = current_items_display_df["material"].fillna("")
                    current_items_display_df["vendor"] = current_items_display_df["vendor"].fillna("")
                    current_items_display_df["quantity"] = pd.to_numeric(current_items_display_df["quantity"], errors="coerce").fillna(0.0)
                    current_items_display_df["unit_price"] = pd.to_numeric(current_items_display_df["unit_price"], errors="coerce").fillna(0.0)
                    current_items_display_df = current_items_display_df.groupby(
                        ["item_number", "material", "vendor", "unit_price"],
                        dropna=False,
                        as_index=False,
                    ).agg({"quantity": "sum"})
                    current_items_display_df = current_items_display_df.sort_values(
                        by=["item_number", "material", "vendor", "unit_price"],
                        ascending=[True, True, True, True],
                    )
                    st.caption(f"{len(current_items_display_df)} item(s) | Total {money(current_total)}")
                    current_items_display_df = current_items_display_df.rename(columns={
                        ...
                        if selected_material_id is None:
                            ...
                            with st.form("edit_material_line_form"):
                                ...
                                if save_line:
                                    ...
                                    merge_matching_material_lines(selected_line_id)
                                ...
                                if delete_line:
                                    ...
                                    if deleted_count <= 0:
                                        st.warning("No matching item was deleted.")
                                    else:
