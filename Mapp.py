...
def search_material_selector(search_text: str, limit: int = 50) -> pd.DataFrame:
    ...
    exact_numeric = digits_only if digits_only else None
    exact_text = term
    ...
    sql = """
        ...
        FROM (
            SELECT
                ...
                ROW_NUMBER() OVER (
                    ...
                    ORDER BY
                        CASE
                            WHEN %s IS NOT NULL AND COALESCE(mvc.vendor_item_number, '') = %s THEN 0
        ...
        ORDER BY
            CASE
                WHEN %s IS NOT NULL AND COALESCE(vendor_item_number, '') = %s THEN 0
