...
def main() -> None:
    ...
    with st.sidebar:
        ...
        st.markdown("### Reports")
        report_labels = ["   Vendor", "   Project Detail"]
        ...
        selected_report_label = st.radio(
            "",
            report_labels,
            index=report_index if report_index is not None else 0,
            key="sidebar_report_nav",
            label_visibility="collapsed",
        )
        if current_page in report_pages:
            page = selected_report_label.strip()

        if admin_pages:
            st.markdown("### Admin")
            admin_index = 0 if current_page == "Admin" else 0
            selected_admin_page = st.radio(
                "",
                admin_pages,
                index=admin_index,
                key="sidebar_admin_nav",
                label_visibility="collapsed",
            )
            if current_page == "Admin":
                page = selected_admin_page
