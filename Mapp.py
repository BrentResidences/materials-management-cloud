            ...
            for table in page.extract_tables() or []:
                for raw_row in table:
                    if not raw_row or len(raw_row) < 4:
                        continue
                    item_no = clean_cell(raw_row[0])
                    description = clean_cell(raw_row[1])
                    material_name = clean_cell(raw_row[2])
                    retail_price = clean_cell(raw_row[3])

                    if item_no.lower() == "item #":
                        continue

                    # continuation rows from page breaks
                    if not item_no and rows and (description or material_name) and not retail_price:
                        if description:
                            rows[-1]["description"] = (rows[-1]["description"] + " " + description).strip()
                        if material_name:
                            rows[-1]["material_name"] = (rows[-1]["material_name"] + " " + material_name).strip()
                        continue

                    if not description and not material_name:
                        continue

                    price_value = None
                    if retail_price:
                        try:
                            price_value = float(retail_price.replace("$", "").replace(",", "").strip())
                        except Exception:
                            price_value = None

                    normalized_item = item_no if item_no != '-' else ''
                    rows.append({
                        "vendor_item_number": normalized_item,
                        "description": description,
                        "material_name": material_name or description,
                        "latest_retail_price": price_value,
                    })
...
def import_vendor_checklist_df(
    ...
    with conn.cursor() as cur:
        for _, row in import_df.iterrows():
            item_number = str(row.get("vendor_item_number") or "").strip()
...
def page_materials_import_vendor_pdf() -> None:
    st.subheader("Import Vendor Checklist PDF")
    st.caption("Upload a checklist PDF with columns: Item #, Description, Name, Retail Price.")

    lookups = get_lookup_data()
    categories = lookups["active_categories"].copy()
    units = lookups["active_units"].copy()

    uploaded_pdf = st.file_uploader("Vendor checklist PDF", type=["pdf"], key="vendor_checklist_pdf_upload")
    if not uploaded_pdf:
        st.info("Upload a PDF to preview and import items.")
        return
    ...
    if parsed_df.empty:
        st.error("No importable rows were found in that PDF.")
        return
    ...
    with st.form("import_vendor_checklist_form"):
        ...
        vendor_name = v1.text_input("Vendor Name", value="Lowes")
    ...
    if len(parsed_df) > 50:
        st.caption(f"Showing first 50 of {len(parsed_df)} parsed rows.")
