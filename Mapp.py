...
def build_master_material_list_pdf(list_id: int) -> BytesIO:
    list_row = query_one("SELECT * FROM master_material_lists WHERE list_id = %s", (list_id,))
    if list_row is None:
        raise ValueError("Master Material List not found.")
    ...
    story.append(Paragraph(str(list_row.get("list_name") or "Master Material List"), styles["Title"]))
    if list_row.get("project_name") or list_row.get("sub_project_name"):
        story.append(Paragraph(f"Project: {list_row.get('project_name') or ''} | Sub-Project: {list_row.get('sub_project_name') or ''}", styles["Normal"]))
    if list_row.get("notes"):
        story.append(Paragraph(str(list_row.get("notes") or ""), styles["Normal"]))
    story.append(Spacer(1, 12))

    if rows.empty:
        story.append(Paragraph("No materials have been added to this master list.", styles["Normal"]))
    else:
        for category_name, cat_df in rows.groupby("category_name", dropna=False):
            cat_label = category_name if pd.notna(category_name) and str(category_name).strip() else "Uncategorized"
            data = [["Category", "Item #", "Item Name", "Vendor", "Price"]]
            ...
            for _, row in cat_df.iterrows():
                data.append([
                    ...
                    row.get("vendor_name") or "",
                    money(row.get("latest_price") or 0),
                ])
            table = Table(data, repeatRows=1, colWidths=[130, 100, 310, 130, 90])
            table.setStyle(TableStyle([
                ("BACKGROUND", (0, 0), (-1, 0), colors.lightgrey),
                ("GRID", (0, 0), (-1, -1), 0.25, colors.grey),
                ("VALIGN", (0, 0), (-1, -1), "TOP"),
...
def build_master_material_list_excel(list_id: int) -> BytesIO:
    list_row = query_one("SELECT * FROM master_material_lists WHERE list_id = %s", (list_id,))
    if list_row is None:
        raise ValueError("Master Material List not found.")
    ...
    if df.empty:
        out_df = pd.DataFrame(columns=["Category", "Item #", "Item Name", "Vendor", "Price"])
    else:
        out_df = df[["category_name", "item_number", "item_name", "vendor_name", "latest_price"]].rename(
            columns={
    ...
    buffer = BytesIO()
    with pd.ExcelWriter(buffer, engine="openpyxl") as writer:
        out_df.to_excel(writer, index=False, sheet_name="Master Material List")
        ws = writer.book["Master Material List"]
        ws.freeze_panes = "A2"
        ws.auto_filter.ref = ws.dimensions
        widths = {"A": 22, "B": 18, "C": 45, "D": 22, "E": 14}
        for col, width in widths.items():
            ws.column_dimensions[col].width = width
        for cell in ws[1]:
            cell.font = cell.font.copy(bold=True)
...
def page_master_material_lists() -> None:
    st.header("Master Material Lists")
    st.caption("Create reusable material checklist/reference lists. These lists do not use quantities or totals.")

    lists_df = query_df(
        """
        SELECT list_id, list_name, project_name, sub_project_name, notes, active, date_created, date_modified
        FROM master_material_lists
        ORDER BY list_name
        """
    )

    create_tab, manage_tab, report_tab = st.tabs(["Create List", "Manage List Items", "Report"])

    with create_tab:
        st.subheader("Create Master Material List")
        with st.form("create_master_material_list_form"):
            list_name = st.text_input("List name", placeholder="Residences at Linwood - Make Ready Item List")
            c1, c2 = st.columns(2)
            project_name = c1.text_input("Project / Property name", placeholder="Residences at Linwood")
            sub_project_name = c2.text_input("Sub-Project / List type", placeholder="Make Ready Item List")
            notes = st.text_area("Notes")
            active = st.checkbox("Active", value=True)
            submitted = st.form_submit_button("Create Master Material List")
            if submitted:
                if not list_name.strip():
                    ...
                    try:
                        execute(
                            """
                            INSERT INTO master_material_lists (list_name, project_name, sub_project_name, notes, active, date_modified)
                            VALUES (%s, %s, %s, %s, %s, %s)
                            """,
                            (list_name.strip(), project_name.strip() or None, sub_project_name.strip() or None, notes.strip() or None, 1 if active else 0, now_ts()),
                        )
                        st.success("Master material list created.")
                        st.rerun()
                    except Exception as exc:
                        st.error(f"Could not create list. The list name may already exist. {exc}")

        if not lists_df.empty:
            st.markdown("**Existing Master Material Lists**")
            st.dataframe(format_dates(lists_df), use_container_width=True, hide_index=True)

    with manage_tab:
        st.subheader("Add / Edit Master Material List Items")
        if lists_df.empty:
            st.info("Create a Master Material List first.")
        else:
            active_lists_df = lists_df[lists_df["active"].fillna(1).astype(int) == 1].copy()
            if active_lists_df.empty:
                active_lists_df = lists_df.copy()
            list_options = {f"{row['list_name']} | ID {int(row['list_id'])}": int(row['list_id']) for _, row in active_lists_df.iterrows()}
            selected_list_label = st.selectbox("Select Master Material List", list(list_options.keys()), key="master_material_manage_select")
            ...
            st.caption(f"Project: {selected_list.get('project_name') or ''} | Sub-Project: {selected_list.get('sub_project_name') or ''}")

            with st.expander("Edit List Name / Header", expanded=False):
                with st.form("edit_master_material_list_form"):
                    edit_list_name = st.text_input("List name", value=selected_list.get("list_name") or "")
                    e1, e2 = st.columns(2)
                    edit_project_name = e1.text_input("Project / Property name", value=selected_list.get("project_name") or "")
                    edit_sub_project_name = e2.text_input("Sub-Project / List type", value=selected_list.get("sub_project_name") or "")
                    ...
                    save_list = st.form_submit_button("Save List Changes")
                    if save_list:
                        if not edit_list_name.strip():
                            ...
                            execute(
                                ...
                                SET list_name = %s, project_name = %s, sub_project_name = %s, notes = %s, active = %s, date_modified = %s
                                WHERE list_id = %s
                                """,
                                (edit_list_name.strip(), edit_project_name.strip() or None, edit_sub_project_name.strip() or None, edit_notes.strip() or None, 1 if edit_active else 0, now_ts(), selected_list_id),
                            )
                            st.success("List updated.")
                            st.rerun()

            st.markdown("**Add Material To This Master List**")
            ...
            if not matches.empty:
                material_options = {
                    f"ID {int(row['material_id'])} | {row.get('material_name') or ''} | {row.get('vendor_item_number') or ''} | {row.get('vendor_name') or ''} | {row.get('full_description') or ''}": int(row["material_id"])
                ...
                with st.form("add_master_material_list_item_form"):
                    ...
                    sort_order = st.number_input("Sort order (optional)", min_value=0, value=0, step=1)
                    item_notes = st.text_input("Line notes")
                    add_item = st.form_submit_button("Add Item To Master List")
                    if add_item:
                        execute(
                            """
                            INSERT INTO master_material_list_items (list_id, material_id, material_vendor_id, sort_order, notes, active, date_modified)
                            VALUES (%s, %s, %s, %s, %s, 1, %s)
                            """,
                            (selected_list_id, selected_material_id, vendor_options[selected_vendor_label], int(sort_order) if sort_order else None, item_notes.strip() or None, now_ts()),
                        )
                        st.success("Item added to Master Material List.")
                        st.rerun()

            st.markdown("**Current Items In This Master List**")
            items_df = _master_material_list_items_df(selected_list_id)
            if items_df.empty:
                st.info("No items have been added to this master list yet.")
            else:
                display_df = items_df[["list_item_id", "category_name", "item_number", "item_name", "vendor_name", "latest_price"]].rename(
                    columns={
                        "list_item_id": "Line ID",
                        "category_name": "Category",
                        "item_number": "Item #",
                        "item_name": "Item Name",
                        "vendor_name": "Vendor",
                ...
                delete_options = {f"Line {int(row['list_item_id'])} | {row.get('item_name') or ''} | {row.get('item_number') or ''}": int(row["list_item_id"]) for _, row in items_df.iterrows()}
                with st.form("delete_master_material_list_item_form"):
                    delete_label = st.selectbox("Select item to remove from this master list", list(delete_options.keys()))
                    remove_item = st.form_submit_button("Remove Selected Item")
                    if remove_item:
                        execute(
                            "UPDATE master_material_list_items SET active = 0, date_modified = %s WHERE list_item_id = %s",
                            (now_ts(), delete_options[delete_label]),
                        )
                        st.success("Item removed from Master Material List.")
                        st.rerun()

    with report_tab:
        st.subheader("Master Material List Report")
        if lists_df.empty:
            st.info("Create a Master Material List first.")
        else:
            active_lists_df = lists_df[lists_df["active"].fillna(1).astype(int) == 1].copy()
            if active_lists_df.empty:
                active_lists_df = lists_df.copy()
            list_options = {f"{row['list_name']} | ID {int(row['list_id'])}": int(row['list_id']) for _, row in active_lists_df.iterrows()}
            selected_report_label = st.selectbox("Select Master Material List", list(list_options.keys()), key="master_material_report_select")
            ...
            if report_items_df.empty:
                ...
                display_df = report_items_df[["category_name", "item_number", "item_name", "vendor_name", "latest_price"]].rename(
                    columns={
                        "category_name": "Category",
                        "item_number": "Item #",
                        "item_name": "Item Name",
            ...
            pdf_key = f"master_material_list_pdf_{selected_report_list_id}"
            xlsx_key = f"master_material_list_xlsx_{selected_report_list_id}"
            if col1.button("Prepare Master Material List PDF", key=f"prep_master_material_list_pdf_{selected_report_list_id}"):
                st.session_state[pdf_key] = build_master_material_list_pdf(selected_report_list_id).getvalue()
            if pdf_key in st.session_state:
                col1.download_button(
                    "Download Master Material List PDF",
                    data=st.session_state[pdf_key],
                    file_name=f"master_material_list_{selected_report_list_id}.pdf",
                    mime="application/pdf",
                    key=f"download_master_material_list_pdf_{selected_report_list_id}",
                )
            if col2.button("Prepare Master Material List Excel", key=f"prep_master_material_list_xlsx_{selected_report_list_id}"):
                st.session_state[xlsx_key] = build_master_material_list_excel(selected_report_list_id).getvalue()
            if xlsx_key in st.session_state:
                col2.download_button(
                    "Download Master Material List Excel",
                    data=st.session_state[xlsx_key],
                    file_name=f"master_material_list_{selected_report_list_id}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    key=f"download_master_material_list_xlsx_{selected_report_list_id}",
                )
...
def main() -> None:
    ...
    report_pages = ["Vendor", "Project Detail", "Master Material Lists"]
    ...
    with st.sidebar:
        ...
        if st.button("Project Detail", key="sidebar_project_detail_btn", use_container_width=True):
            page = "Project Detail"
            st.session_state["sidebar_selected_page"] = page
        if st.button("Master Material Lists", key="sidebar_master_material_lists_btn", use_container_width=True):
            page = "Master Material Lists"
            st.session_state["sidebar_selected_page"] = page
