    ...
    if materials_section == "Add Vendor Info":
        ...
        if mats.empty:
            ...
            if vendor_edit_all_df.empty:
                ...
                material_name_lookup = {int(row['material_id']): str(row['material_name']) for _, row in mats.iterrows()}
                ...
                selected_vendor_material_name = material_name_lookup.get(selected_vendor_material_id)
                material_labels = list(mat_map.keys())
                default_material_index = material_labels.index(selected_vendor_material_name) if selected_vendor_material_name in material_labels else 0

                with st.form("add_vendor_info_edit_form"):
                    edit_material_name = st.selectbox(
                        "Material",
                        material_labels,
