            ...
            if search.strip():
                sql += """
                    AND (
                        ...
                        OR EXISTS (
                            SELECT 1
                            FROM material_vendor_current mvc_search
                            WHERE mvc_search.material_id = m.material_id
                              AND (
                                    COALESCE(mvc_search.vendor_item_number, '') ILIKE %s
                                 OR COALESCE(mvc_search.vendor_name, '') ILIKE %s
                              )
                        )
                    )
                """
