...
def render_material_master_editor(material_id: int, lookups: dict[str, pd.DataFrame], key_prefix: str, expanded: bool = True, title: str = "Material Details") -> None:
    ...
    with st.expander(title, expanded=expanded):
        ...
        with st.expander("Delete Material", expanded=False):
            ...
            subproject_line_count = int(dependency_counts.get("subproject_lines") or 0)
            if subproject_line_count > 0:
                st.error(
                    "This material is already used on one or more Sub-Project material lines. "
                    "Delete or remove those Sub-Project material lines before permanently deleting this material."
                )

            confirm_delete = st.checkbox(
                "Yes, permanently delete this material",
                key=f"{effective_key_prefix}_confirm_delete",
            )
            delete_material = st.button(
                "Delete Material",
                key=f"{effective_key_prefix}_delete_material",
                disabled=(not confirm_delete or subproject_line_count > 0),
            )
            if delete_material:
                conn = get_conn()
                try:
                    with conn.cursor() as cur:
                        cur.execute(
                            "DELETE FROM master_material_list_items WHERE material_id = %s",
                            (material_id,),
                        )
                        cur.execute(
                            """
        ...
        if cats.empty or units.empty:
            ...
            if not edit_search_term:
                ...
                edit_materials_df = query_df(
                    ...
                    LIMIT 100
                    """,
                    (edit_like, edit_like, edit_like, edit_like, edit_like, edit_like, edit_like, edit_like, edit_digits, edit_digits_like),
                )
