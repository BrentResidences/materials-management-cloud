...
def page_materials(materials_section: str | None = None) -> None:
    ...
    if materials_section == "Add Material":
        ...
        st.caption(
            "Build 3 workflow: start with the actual vendor product/SKU, then either link it to an existing product profile "
            "or create a new product profile from the same screen."
        )
        ...
        if cats.empty or units.empty:
            ...
            mode = st.radio(
                ...
                horizontal=True,
                key="vendor_product_profile_mode",
            )
            ...
            if mode == "Link to Existing Product Profile":
                st.markdown("#### 1. Find the existing product profile")
                profile_search = st.text_input(
                    "Search existing product profiles",
                    key="vendor_first_profile_search",
                    placeholder="Example: 30 white shaker vanity, washer 4-cu ft, model number, category, or Material ID",
                )
                profile_choices_df = filter_material_selector_df(mats, profile_search)
                if profile_search.strip() and profile_choices_df.empty:
                    st.warning("No matching product profile found. Use Create New Product Profile if this is not the same exact product.")
                elif not profile_search.strip():
                    st.info("Type a material name, description, manufacturer, model, category, or Material ID to find an existing profile.")
                else:
                    profile_options = {
                    ...
                    selected_profile_label = st.selectbox(
                        "Select matching existing product profile",
                        list(profile_options.keys()),
                        key="vendor_first_existing_profile_select",
                    )
                    ...
                    st.caption(
                        "Use this only when the vendor item is the same product/profile already saved. "
                        "If it is only similar but has a different manufacturer, model, look, or specifications, create a new product profile."
                    )

            st.markdown("#### 2. Enter vendor product information")
            with st.form("vendor_product_first_form"):
                v1, v2, v3 = st.columns(3)
                vendor_name = v1.text_input("Vendor name")
                vendor_item_number = v2.text_input("Vendor item number / SKU")
                latest_retail_price = v3.number_input("Price", min_value=0.0, step=0.01)

                v4, v5, v6 = st.columns(3)
                vendor_store_number = v4.text_input("Store number")
                store_aisle = v5.text_input("Store aisle")
                latest_retail_price_date = v6.date_input("Price date", format="MM/DD/YYYY")

                vendor_notes = st.text_input("Vendor notes")

                if mode == "Create New Product Profile":
                    st.markdown("#### New product profile details")
                    p1, p2 = st.columns(2)
                    material_name = p1.text_input("Material name / internal short name")
                    internal_material_code = p2.text_input("Internal material code", value=normalize_vendor_item_number(vendor_item_number))
                    full_description = st.text_area("Full description / vendor or manufacturer description")

                    p3, p4, p5 = st.columns(3)
                    selected_cat = p3.selectbox("Category", list(cat_map.keys()), key="vendor_first_new_cat")
                    ...
                    selected_sub = p4.selectbox("Sub-category", list(sub_map.keys()) if sub_map else [""], key="vendor_first_new_sub")
                    default_unit = p5.selectbox("Default unit", unit_names, key="vendor_first_new_unit")

                    p6, p7, p8 = st.columns(3)
                    manufacturer = p6.text_input("Manufacturer")
                    model_number = p7.text_input("Model number")
                    dimension_display = p8.text_input("Dimensions / size display")
                    material_notes = st.text_area("Product profile notes")
                else:
                    st.markdown("#### Optional vendor product override details")
                    st.caption(
                        "Leave these blank when this vendor SKU is the same exact product as the selected product profile. "
                        "Enter them only if the vendor has a more exact description or product-specific notes you want saved on the SKU."
                    )
                    vendor_full_description = st.text_area("Vendor's exact product description (optional)")
                    ov1, ov2 = st.columns(2)
                    vendor_manufacturer = ov1.text_input("Vendor product manufacturer (optional)")
                    vendor_model_number = ov2.text_input("Vendor product model number (optional)")

                st.markdown("#### Product photo")
                st.caption("Photos are saved to the exact vendor product/SKU. If this is the same exact product, you can reuse or upload the same-looking photo.")
                vendor_photos = st.file_uploader(
                    "Upload vendor product photo(s) / screenshot image file(s)",
                    type=["png", "jpg", "jpeg", "webp"],
                    accept_multiple_files=True,
                    key="vendor_first_product_photos",
                )
                vendor_photo_caption = st.text_input("Photo caption / notes")

                submitted = st.form_submit_button("Save Vendor Product")
            ...
            st.caption("Use this area to edit the shared product profile fields when needed. Vendor-specific SKU, price, photo, manufacturer/model overrides stay on the vendor product record.")
