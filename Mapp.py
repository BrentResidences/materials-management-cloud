...
def page_materials(materials_section: str | None = None) -> None:
    ...
    if materials_section == "Add Material":
        ...
        if cats.empty or units.empty:
            ...
            st.divider()
            st.subheader("Edit Existing Material")
            edit_material_search = st.text_input(
                ...
                help="Start typing to search. The matching material list will appear after you enter search text.",
            )
            edit_search_term = (edit_material_search or "").strip()
            if not edit_search_term:
                st.info("Type a material name, full description, item number, manufacturer, model, category, or Material ID to find the material you want to edit.")
            else:
                ...
                if edit_materials_df.empty:
                    st.info("No matching material was found. If this is a new material, enter the full material information in the Add New Material section above.")
                else:
                    material_options = {}
                    for _, row in edit_materials_df.iterrows():
                        material_name_label = str(row.get('material_name') or '').strip()
                        material_desc_label = str(row.get('full_description') or '').strip()
                        category_label = str(row.get('category_name') or '').strip()
                        vendor_label = str(row.get('vendor_name') or '').strip()
                        item_number_label = str(row.get('vendor_item_number') or '').strip()
                        label_parts = [material_name_label, f"ID {int(row['material_id'])}"]
                        if item_number_label:
                            label_parts.append(f"Item # {item_number_label}")
                        if vendor_label:
                            label_parts.append(vendor_label)
                        if category_label:
                            label_parts.append(category_label)
                        if material_desc_label:
                            label_parts.append(material_desc_label[:120])
                        material_options[" | ".join(label_parts)] = int(row['material_id'])
                    ...
                    selected_material_label = st.selectbox(
                        ...
                        help="Choose the exact material record to edit from the matches below.",
                    )
                    selected_material_id = material_options[selected_material_label]
...
def main() -> None:
    ...
    page_dispatch = {
        ...
        "Add / Edit Materials": (page_materials, "Add Material"),
        "Add Vendor Materials Info": (page_materials, "Add Vendor Info"),
