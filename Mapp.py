...
def page_materials(materials_section: str | None = None) -> None:
    ...
    if materials_section == "Search / Review":
        ...
        if run_search:
            sql = """
                WITH vendor_ranked AS (
                    SELECT
                        ...
                        COALESCE(NULLIF(mvc.latest_quoted_price, 0), mvc.latest_retail_price, 0) AS latest_price,
                        ROW_NUMBER() OVER (
                            PARTITION BY mvc.material_id
                            ORDER BY
                                CASE WHEN COALESCE(mvc.vendor_item_number, '') <> '' THEN 0 ELSE 1 END,
                                CASE WHEN COALESCE(mvc.vendor_name, '') <> '' THEN 0 ELSE 1 END,
                ...
                SELECT
                    ...
                    COALESCE(vr.vendor_item_number, '') AS item_number,
                    m.material_name,
                    COALESCE(vr.vendor_name, '') AS vendor,
                    COALESCE(vr.latest_price, 0) AS latest_price
                FROM materials m
                LEFT JOIN company_categories c ON m.category_id = c.category_id
                LEFT JOIN company_subcategories s ON m.subcategory_id = s.subcategory_id
                LEFT JOIN vendor_ranked vr ON m.material_id = vr.material_id AND vr.rn = 1
                WHERE m.active = 1
            """
            params: list[Any] = []

            if search.strip():
                sql += " AND (m.material_name ILIKE %s OR m.full_description ILIKE %s OR m.manufacturer ILIKE %s OR m.model_number ILIKE %s OR m.dimension_display ILIKE %s OR COALESCE(vr.vendor_item_number, '') ILIKE %s OR COALESCE(vr.vendor_name, '') ILIKE %s)"
                like = f"%{search.strip()}%"
            ...
            sql += " ORDER BY COALESCE(c.category_name, ''), COALESCE(vr.vendor_item_number, ''), m.material_name"
            df_mat_search = query_df(sql, tuple(params))
            if not df_mat_search.empty:
                if 'item_number' in df_mat_search.columns:
                    df_mat_search['item_number'] = df_mat_search['item_number'].apply(normalize_vendor_item_number)
                if 'latest_price' in df_mat_search.columns:
                    df_mat_search['latest_price'] = pd.to_numeric(df_mat_search['latest_price'], errors='coerce').fillna(0.0)
                df_mat_search = df_mat_search.rename(columns={
                    'category': 'Category',
                    'item_number': 'Item Number',
                ...
                df_mat_search = df_mat_search[['Category', 'Item Number', 'Material Name', 'Vendor', 'Latest Price']]
