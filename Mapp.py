...
def render_material_master_editor(material_id: int, lookups: dict[str, pd.DataFrame], key_prefix: str, expanded: bool = True, title: str = "Material Details") -> None:
    ...
    with st.expander(title, expanded=expanded):
        ...
        edit_manufacturer = e5.text_input(
            ...
            key=f"{effective_key_prefix}_manufacturer",
        )
        edit_model_number = e6.text_input(
            "Model number",
            value=material_row.get("model_number") or "",
            key=f"{effective_key_prefix}_model",
        )
        ...
        render_material_photo_manager(material_id, effective_key_prefix)
...
def page_materials(materials_section: str | None = None) -> None:
    ...
    if materials_section == "Add Material":
        ...
        if cats.empty or units.empty:
            ...
            st.markdown("**Optional Material Screenshot**")
            st.caption("Click in the paste box, press Ctrl+V, then complete and submit the Add Material form below.")
            add_material_pasted_value = render_clipboard_paste_area("add_material_paste_component")
            if add_material_pasted_value:
                decoded_preview = decode_pasted_image(add_material_pasted_value)
                if decoded_preview:
                    st.image(decoded_preview[2], caption="Pasted screenshot preview", width=240)
            ...
            with st.form("add_material_form"):
                ...
                st.caption("Material photos can be uploaded here, or pasted in the screenshot paste area above before saving this material.")
                material_photos = st.file_uploader(
                    "Upload material photo(s) / screenshot image file(s)",
                    type=["png", "jpg", "jpeg", "webp"],
                    accept_multiple_files=True,
                    key="add_material_photos",
                    help="You can browse for image files or drag/drop screenshot image files here.",
                )
                material_photo_caption = st.text_input("Photo caption / notes", key="add_material_photo_caption")
                ...
                if submitted and material_name.strip():
                    ...
                    try:
                        with conn.cursor() as cur:
                            ...
                            for uploaded_photo in material_photos or []:
                                cur.execute(
                                    """
                                    INSERT INTO material_photos (
                                        material_id, file_name, content_type, photo_bytes, caption, active, date_modified
                                    ...
                                    (
                                        ...
                                        material_photo_caption.strip() or None,
                                        now_ts(),
                                    ),
                                )
                            pasted_decoded = decode_pasted_image(st.session_state.get("add_material_paste_component_last_paste"))
                            if pasted_decoded:
                    ...
                    get_lookup_data.clear()
                    get_dashboard_data.clear()
                    st.session_state.pop("add_material_paste_component_last_paste", None)
    ...
    if materials_section == "Add Vendor Info":
        ...
        if mats.empty:
            ...
            if add_material_choices_df.empty:
                ...
                add_vendor_pasted_value = render_clipboard_paste_area("add_vendor_paste_component")
                    ...
                    if submitted and vendor_name.strip():
                        ...
                        try:
                            with conn.cursor() as cur:
                                ...
                                pasted_decoded = decode_pasted_image(st.session_state.get("add_vendor_paste_component_last_paste"))
                                if pasted_decoded:
                                    pasted_file_name, pasted_content_type, pasted_photo_bytes = pasted_decoded
                                    cur.execute(
                                        """
                        ...
                        get_lookup_data.clear()
                        get_dashboard_data.clear()
                        st.session_state.pop("add_vendor_paste_component_last_paste", None)
