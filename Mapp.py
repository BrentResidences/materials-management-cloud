...
def build_project_report_pdf(project_id: int) -> BytesIO:
    ...
    if rows.empty:
        ...
        for category_name, cat_df in rows.groupby("category_name", dropna=False):
            ...
            category_block = [Paragraph(f"Category: {cat_label}", styles["Heading2"])]
            ...
            category_block.append(table)
            category_block.append(Paragraph(f"Total {cat_label}: {money(cat_total)}", styles["Normal"]))
            category_block.append(Spacer(1, 10))
