...
def page_materials_edit_materials() -> None:
    st.subheader("Edit Materials")
    st.caption("Search by item number or material name, then edit the material record including category and sub-category.")

    lookups = get_lookup_data()
    search_value = st.text_input("Search by item number or material name", key="materials_edit_page_search")
    ...
    selected_label = st.selectbox("Select material", list(result_options.keys()), key="materials_edit_page_select")
    ...
    primary_vendor = query_one(
        """
        SELECT vendor_name, vendor_item_number
        FROM material_vendor_current
        WHERE material_id = %s AND active = 1
        ORDER BY vendor_name, vendor_item_number, material_vendor_id
        LIMIT 1
        """,
    ...
    s1.caption(f"Selected: {material_row.get('material_name') or ''}")
    s2.caption(f"Item #: {normalize_vendor_item_number((primary_vendor or {}).get('vendor_item_number')) or 'None'}")
    s3.caption(f"Vendor: {(primary_vendor or {}).get('vendor_name') or 'None'}")
    ...
    selected_category_name = st.selectbox(
        "Category",
        list(category_options_edit.keys()),
        index=list(category_options_edit.keys()).index(current_category_name),
        key=f"materials_edit_page_category_{selected_material_id}",
    )
    selected_category_id = category_options_edit[selected_category_name]
    ...
    if current_sub_name not in sub_options_edit:
        current_sub_name = "No Sub-category"
    ...
    with st.form(f"materials_edit_page_form_{selected_material_id}"):
        a1, a2 = st.columns(2)
        ...
        b1, b2 = st.columns(2)
        edit_sub_name = b1.selectbox(
            "Sub-category",
            list(sub_options_edit.keys()),
            index=list(sub_options_edit.keys()).index(current_sub_name),
            key=f"materials_edit_page_sub_{selected_material_id}",
        )
        edit_unit_name = b2.selectbox(
            "Default unit",
            list(unit_options_edit.keys()),
            index=list(unit_options_edit.keys()).index(current_unit_name),
            key=f"materials_edit_page_unit_{selected_material_id}",
        )
        ...
        edit_notes = st.text_area("Notes", value=material_row.get("notes") or "", key=f"materials_edit_page_notes_{selected_material_id}")
        edit_active = st.checkbox("Active", value=bool(material_row.get("active", 1)), key=f"materials_edit_page_active_{selected_material_id}")
...
def page_materials(materials_section: str | None = None) -> None:
    ...
    if materials_section is None:
        materials_section = st.radio("Materials Section", ["Add Material", "Add Vendor Info", "Edit Materials", "Search / Review", "Vendor Master Reports", "Import Vendor Checklist PDF"], horizontal=True, key="materials_section")
    ...
    if materials_section == "Edit Materials":
        page_materials_edit_materials()
...
def page_projects(project_section: str | None = None) -> None:
    ...
    if project_section == "Add Materials to Sub-Project":
        work_items = query_df(
            """
            SELECT wi.work_item_id, p.project_name, wi.work_item_name,
                   p.project_name || ' | ' || wi.work_item_name AS label
            FROM project_work_items wi
            JOIN projects p ON wi.project_id = p.project_id
            ORDER BY p.project_name, wi.sort_order NULLS LAST, wi.work_item_name
            """
        ...
        units = lookups["active_units"].copy()
        if work_items.empty or units.empty:
            ...
            with top1:
                work_item_label = st.selectbox("Sub-Project", list(wi_map.keys()), key="add_line_work_item_select")
                work_item_id = wi_map[work_item_label]
                ...
                st.caption(f"Project: {work_item_row['project_name']}")
                if work_item_row.get("work_item_description"):
                    st.caption(f"Description: {work_item_row['work_item_description']}")

                search_value = st.text_input(
                    "Search material by item number or material name",
                    key="subproject_material_search",
                ...
                if current_lines_df.empty:
                    ...
                    st.dataframe(current_lines_df, use_container_width=True, hide_index=True, height=260)
                ...
                if not search_df.empty:
                    ...
                    selected_search_label = st.selectbox(
                        ...
                        key="material_search_result_select",
                    )
                    selected_search_row = search_df.loc[search_df["display_label"] == selected_search_label].iloc[0].to_dict()

            with top2:
                if search_df.empty or selected_search_row is None:
                    st.info("Select a search result to populate the material line form.")
                else:
                    ...
                    with st.expander("Edit Selected Material", expanded=False):
                        ...
                        selected_edit_category_name = st.selectbox(
                            ...
                            key=f"subproject_edit_mat_category_{selected_material_id}",
                        )
                        selected_edit_category_id = category_options_edit[selected_edit_category_name]

                        sub_options_edit = {"No Sub-category": None}
                        if selected_edit_category_id is not None and not subs_all.empty:
                            filtered_subs = subs_all[subs_all["category_id"] == selected_edit_category_id]
                        else:
                            filtered_subs = subs_all.iloc[0:0] if not subs_all.empty else pd.DataFrame()
                        for _, row in filtered_subs.iterrows():
                            sub_options_edit[str(row["subcategory_name"])] = int(row["subcategory_id"])
                        ...
                        edit_sub_name = e3.selectbox(
                            ...
                            key=f"subproject_edit_mat_sub_{selected_material_id}",
                        )
                        edit_unit_name = e4.selectbox(
                            "Default unit",
                            unit_labels,
                            index=default_unit_edit_index,
                            key=f"subproject_edit_mat_unit_{selected_material_id}",
                        )
                        e5, e6, e7 = st.columns(3)
                        edit_manufacturer = e5.text_input(
                            "Manufacturer",
                            value=material_row.get("manufacturer") or "",
                            key=f"subproject_edit_mat_manufacturer_{selected_material_id}",
                        )
                        edit_model_number = e6.text_input(
                            "Model number",
                            value=material_row.get("model_number") or "",
                            key=f"subproject_edit_mat_model_{selected_material_id}",
                        )
                        edit_dimension_display = e7.text_input(
                            "Dimensions display",
                            value=material_row.get("dimension_display") or "",
                            key=f"subproject_edit_mat_dimensions_{selected_material_id}",
                        )
                        edit_notes = st.text_area(
                            "Notes",
                            value=material_row.get("notes") or "",
                            key=f"subproject_edit_mat_notes_{selected_material_id}",
                        )
                        edit_active = st.checkbox(
                            "Active",
                            value=bool(material_row.get("active", 1)),
                            key=f"subproject_edit_mat_active_{selected_material_id}",
                        )
                        save_material_changes = st.button(
                            "Save Material Changes",
                            key=f"subproject_save_material_changes_{selected_material_id}",
                        )
                            ...
                            if not edit_material_name.strip():
                                ...
                                execute(
                                    ...
                                    (
                                        ...
                                        edit_material_name.strip(),
                                        edit_full_description.strip() or None,
                                        category_options_edit[selected_edit_category_name],
                                        sub_options_edit[edit_sub_name],
                                        unit_options_edit[edit_unit_name],
                                        edit_manufacturer.strip() or None,
                                        edit_model_number.strip() or None,
...
def main() -> None:
    ...
    page_dispatch = {
        ...
        "Materials - Edit Materials": (page_materials, "Edit Materials"),
